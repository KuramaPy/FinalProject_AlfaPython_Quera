...
technolife_df = pd.read_csv('technolife_data.csv')
torob_df = pd.read_csv('torob_data.csv')
...
print(f"بازه قیمتی داده‌های تکنولایف: {sale_technolife} تومان")
print(f"بازه قیمتی داده‌های ترب: {sale_torob} تومان")

min = min(technolife_df['price'].min(), torob_df['price'].min())
max = max(technolife_df['price'].max(), torob_df['price'].max())

print(f"بازه قیمتی لپ‌تاپ‌های ایسوس: {min} تا {max} تومان")
